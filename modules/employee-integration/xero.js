...
app.get("/xero-callback", async function (req, res) {
    ...
    const response = await xero.payrollAUApi.getSuperfunds(tenant['tenantId']).catch((err) => {console.log(err);});
    
    //Add Super to Tenant
    db.updateTenant({'tenantId': tenant['tenantId']}, 
    {
    ...
    db.createToken(token)
     ...
     db.findRequest({'_id': requestID}, request => {
      ...
      //Send
        mailgun.messages().send(data, (err, body) => { console.log(err)});
      });
...
app.post("/new-employee-request", function (req, res) {
  ...
  app.get("/employee-application", function (req, res) {
    ...
    db.findToken({'refresh_token': tok}, token => {
        db.findRequest({'_id': token['request']}, request => {
            db.findTenant({'tenantId': tenantId}, tenant => {
                res.render("application", {token: token['refresh_token'], request: request, tenant: tenant})
            })
        })
    })
  });
  ...
  app.post("/create-employee", function (req, res) {
    ...
    db.findToken({'refresh_token': req.body.token}, async token => {
        db.findTenant({'tenantId': req.body.tenantId}, async tenant => {
            db.findRequest({'_id': token['request']}, async request => {
                ...
                req.body.superFund == "own" && await xero.payrollAUApi.createSuperfund(tenant.tenantId, { "uSI": usi, "type":"REGULATED"}).catch( err => {console.log(err)});
                ...
                //Create Employee
                const response = await xero.payrollAUApi.createEmployee(tenant.tenantId, employee).catch((error) => {res.send(error);});
